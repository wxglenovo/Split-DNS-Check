...
# 二进制文件初始化
# ===============================
def ensure_bin_file(path, default={}):
    if not os.path.exists(path):
        os.makedirs(os.path.dirname(path), exist_ok=True)
        try:
            with open(path, "wb") as f:
                msgpack.packb(default, f, use_bin_type=True)
            print(f"✅ 已创建 {path}")
...
def download_all_sources():
    ...
    for i in range(PARTS):
        ...
        with open(part_file, "w", encoding="utf-8") as f:
            f.write("\n".join(part_rules))
        print(f"⏱ 分片 {i+1} 写入 {len(part_rules)} 条 -> {part_file}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--force-update", action="store_true", help="强制下载规则并切片")
    args = parser.parse_args()

    # 初始化 hash_list 和 delete_counter
